...
class ResNet9(ImageClassificationBase):
    def __init__(self, in_channels, num_classes, num_conv_blocks=2):
        super().__init__()

        self.conv1 = conv_block(in_channels, 64)
        self.conv2 = conv_block(64, 128, pool=True)
        self.res1 = nn.Sequential(*[conv_block(128, 128) for _ in range(num_conv_blocks)])

        self.conv3 = conv_block(128, 256, pool=True)

        self.conv4 = conv_block(256, 512, pool=True)
        self.res2 = nn.Sequential(*[conv_block(512, 512) for _ in range(num_conv_blocks)])
    ...
    def forward(self, xb):
        ...
        # Add forward pass for additional convolutional layers if needed
        out = self.classifier(out)
        return out


# Instantiate the model
model = ResNet9(1, 4, num_conv_blocks=3)
