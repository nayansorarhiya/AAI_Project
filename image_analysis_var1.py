...
# Datasets paths
train_data_root = './newData/Train'
test_data_root = './newData/Test'
...
class ResNet9WithExtraConv(ImageClassificationBase):
    ...
    def forward(self, xb):
        ...
        out = self.conv5(out)  # Added convolution block
...
model_with_extra_conv = ResNet9WithExtraConv(1, 4)
...
history_with_extra_conv = [evaluate(model_with_extra_conv, val_dl)]
# history
...
# %%time
history_with_extra_conv += fit_one_cycle(epochs, max_lr, model_with_extra_conv, train_dl, val_dl,
...
result = evaluate(model_with_extra_conv, test_loader)
result
...
with torch.no_grad():
    for inputs, labels in test_loader:
        outputs = model_with_extra_conv(inputs)
...
plot_accuracies(history_with_extra_conv)
plot_losses(history_with_extra_conv)
